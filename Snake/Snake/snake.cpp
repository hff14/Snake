...
int Death(int x[Number_Snake], int y[Number_Snake], int fla[Number_Snake])			//判断蛇是否死亡（碰壁或者碰身体）的函数
{
	int sum=0;
...
bool Choose_again()											//GAME OVER后再选择函数，使用Choose_open()。
{
	CloseGraph();
	system("CLS");
	Over_title();
...
bool Game()
{
	...
	for(int i=0;i<Number_Snake;i++)								//首次创建蛇体
	{
		snake[i].Create(i);
		fla[i]=0;
	}
	...
	while(1)
	{
		...
		for(int i=0;i<Number_Snake;i++)
		{
			...
			if(Snake_of_direction[i]==up)								//由以上获得的蛇的行进方向，导出蛇头部的下一坐标
		...
		if(sum!=0)
		{
			...
			{
				if(fla[m]==1) continue;
				if(headx[m]==random_node.x&&heady[m]==random_node.y)	//判断是否吃掉食物，即判断（新）头部是否在食物上，若是，则增加头上的一节。
				{
					snake[m].Grow(random_node);
					
					if(m==0)										//增添蛇体则需要增设颜色区别
